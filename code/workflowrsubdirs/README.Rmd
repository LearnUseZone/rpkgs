...
- This package provides a possibility to build .html pages from their associated .Rmd files
  saved in subdirectories of a \"workflowr\" project.
- Package \"workflowr\" is used for .Rmd files saved in directory \"analysis\" and also
  for any other functionality that package \"workflowr\" offers.
- Package \"workflowrsubdirs\" is used only to build .html pages from their associated .Rmd files
...
## General rules
### Directory structure
- Because this package is a kind of an (optional) extension for package
  \"[workflowr](https://github.com/jdblischak/workflowr)\",
  it's necessary to adhere the structure of
  the main \"workflowr\" project's working directory (hereinafter \"main workflowr dir\").
...
- About parameters
  - patterns
    - If parameter \"patterns\" isn't NULL then it has always to end
      ...
      extension \".rmd\" or \".Rmd\" (case sensitive, \".\" is also required). \
      This is made in accordance to behavior of package \"workflowr\"
...
- Error when .Rmd file saved in \"code-rmd subdirs\" contain a space in its name
  after running e.g. workflowrsubdirs::build_htmls(\"code-rmd/subdir\", F, \"test file.Rmd\"): \
...
- Error when .Rmd file saved in \"analysis\" contain a space in its name
  after running e.g. workflowr::wflow_build(\"analysis/test file.Rmd\"): \
...
- Even when relevant temporary files were committed separately (see above),
  ...
  - Commit of all removed (see below) temporary (helping) .Rmd files and new .html files.
- If other than \"workflowr\" project is originally opened using its .Rproj file then
  function build_htmls() fails because \"base::setwd(here::here())\" in this function
  sets a working directory to an original .Rproj working directory regardless
  a current working directory.
  - Potentially if a relevant .Rproj working directory has the same structure then
    it could work but I didn't test it, yet.
...
1. Uninstall this package from RStudio \"Packages\" tab if it exists there.
...
- Several options can be used to render all those files, for example:
  - workflowrsubdirs::build_htmls()
    - All .Rmd (or .rmd) files in \"code-rmd subdirs\" will be rendered to .html files.
  - workflowrsubdirs::build_htmls(dirs = \"code-rmd/subdir\")
    - All .Rmd (or .rmd) files in directories and subdirectories of \"code-rmd/subdir\" will be rendered to .html files.
  - workflowrsubdirs::build_htmls(dirs = \"code-rmd\\\\subdir\")
    - All .Rmd (or .rmd) files in directories and subdirectories of \"code-rmd/subdir\" will be rendered to .html files.
  - workflowrsubdirs::build_htmls(dirs = \"code-rmd/subdir\", orig_rmd_patterns = \".\*.(r|R)md$\")
  - workflowrsubdirs::build_htmls(dirs = \"code-rmd/subdir\", orig_rmd_patterns = c(\"^test.\*.rmd$\", \"file1.Rmd\", \"-.\*.[ R , r ]md\"))

- Note:
  - A real example with .Rmd files in subdirectories
  (under directory \"codeRmd\" in this case) can be found
...
3. Remember that although commit of temporary .Rmd files were made within function
   workflowrsubdirs::build_htmls(), you still have to commit the rest of files.
