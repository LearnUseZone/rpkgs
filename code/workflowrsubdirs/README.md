...
files. Some examples follow.
  ...
  - If you want to try following options by yourself, remember to 1stly
    run file `rpkgs/rpkgs.Rproj` which also set the correct working
...
#         is processed only once, for example pattern = "plot.{1,2}.[ R , r ]md" points to
#         files "plots.Rmd" and "plotly.Rmd" but "plotly.Rmd" is also specified
#         in the last pattern (so it's specified twice).
...
#         separate commit of temporary analysis .Rmd files which means that after a created
#         .html file is opened, workflowr button will show in "Checks" tab error:
