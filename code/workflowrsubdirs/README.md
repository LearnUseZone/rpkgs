...
This package is used (only) to build HTML files from their associated R
...
6.  Delete temporary R Markdown files in directories `code-rmd` and
    `analysis`.
...
[.R
files](https://github.com/LearnUseZone/rpkgs/tree/master/code/examples/iris),
their associated [R Markdown
files](https://github.com/LearnUseZone/rpkgs/tree/master/code-rmd/examples/iris)
and final [web pages](https://learnusezone.github.io/rpkgs/index.html)
show possible processing of iris dataset using package
`workflowrsubdirs`.  
**The main focus of these examples is on demonstration of the
functionality of package workflowrsubdirs rather than on choosing the
...
1.  After `workflowrsubdirs` is installed, open a `workflowr` project
    (run .Rproj file).
...
If you run `webshot::install_phantomjs()` and then run a relevant
solution again then warning like the following one is shown (in
Console):
...
This issue is also described
[here](https://github.com/hafen/trelliscopejs/issues/61).
